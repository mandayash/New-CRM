...
const EditGiftForm: React.FC<EditGiftFormProps> = ({ initialData, onSubmit, onCancel }) => {
  const fileInputRef = useRef<HTMLInputElement>(null);
  
  // Form state
  const [giftName, setGiftName] = useState(initialData.name);
  const [stockCount, setStockCount] = useState(initialData.stock);
  const [pointsRequired, setPointsRequired] = useState(initialData.points);
  const [image, setImage] = useState<File | null>(null);
  const [preview, setPreview] = useState<string | null>(initialData.imageUrl);
